...
class TestActions(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.d.sentry.wait_for_messages({"etcd": re.compile("Healthy*|Unhealthy*")})
        # cls.d.sentry.wait()
    ...
    def test_health_check(self):
        ...
        outcome = self.d.action_fetch(
            action_id, timeout=7200, raise_on_timeout=True, full_output=True
        )
        self.assertEqual(outcome["status"], "completed")
        self.assertTrue(
            "cluster is healthy" in outcome["results"]["result-map"]["message"]
        )
    ...
    def test_snapshot_restore(self):
        ...
        for dataset in ["v2", "v3"]:
            ...
            filenames[dataset] = os.path.basename(
                outcome["results"]["snapshot"]["path"]
            )
